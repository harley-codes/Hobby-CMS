...
export async function GET(request: Request, { params }: { params: { accessToken: string } })
{
	...
	const project = await client.getProjectDetailsPublicAsync(params.accessToken, getHostFromRequestHeaders(request.headers))
